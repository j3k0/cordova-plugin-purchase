        ...
        export class Utils {
            ...
            static callExternal<F extends Function = Function>(context: ExecutionContext, name: string, callback: F, ...args: any): void {
                try {
                    ...
                    context.log.logCallbackException(name, e as Error);
                }
            }
            ...
            ///
            static ajax(context: ExecutionContext, options: IAjaxOptions): { done: (cb: () => void) => void } {
                ...
                xhr.onreadystatechange = function (/*event*/) {
                    try {
                        if (xhr.readyState === 4) {
                            if (xhr.status === 200) {
                                context.callExternal('ajax.success', options.success as Function, JSON.parse(xhr.responseText));
                            }
                            else {
                                log.warn("ajax -> request to " + options.url + " failed with status " + xhr.status + " (" + xhr.statusText + ")");
                                context.callExternal('ajax.error', options.error as Function, xhr.status, xhr.statusText);
                            }
                        }
                    }
                    catch (e) {
            ...
            static ajaxWithHttpPlugin(context: ExecutionContext, options: IAjaxOptions): { done: (cb: () => void) => void } {
                let doneCb = function () { };
                ...
                const ajaxDone = (response: any) => {
                    try {
                        if (response.status == 200) {
                            context.callExternal('ajax.success', options.success as Function, JSON.parse(response.data));
                        }
                        else {
                            context.log.warn("ajax[http] -> request to " + options.url + " failed with status " + response.status + " (" + response.error + ")");
                            context.callExternal('ajax.error', options.error as Function, response.status, response.error);
                        }
                    }
                    catch (e) {
                        context.log.warn("ajax[http] -> request to " + options.url + " failed with an exception: " + (e as Error).message);
                        if (options.error) context.callExternal('ajax.error', options.error, 417, (e as Error).message);
                    }
                    context.callExternal('ajax.done', doneCb);
                }
            ...
            ///
            static md5(r: any) { return md5(r); }

            static delay(fn: () => void, wait: number) {
